...
def compute_derivatives(
    ...
    ⚠️ Deprecated: 建議使用 compute_gradient() 或 compute_all_gradients()
    
    Args:
        f: 標量場 [batch_size, 1]
        x: 座標 [batch_size, spatial_dim]
        order: 微分階數 (1 或 2)
        
    Returns:
        一階導數: [batch_size, spatial_dim] (order=1)
        二階導數: [batch_size, spatial_dim] 對角項 (order=2)
    """
    spatial_dim = x.shape[1]
    
    if order == 1:
        return compute_all_gradients(f, x, spatial_dim)
    elif order == 2:
        # 計算對角項 (∂²f/∂x₁², ∂²f/∂x₂², ...)
        second_derivs = []
        for i in range(spatial_dim):
            d2f_dxi2 = compute_second_derivative(f, x, i, i, spatial_dim)
...
def compute_gradient_2d(
    ...
    ⚠️ Deprecated: 建議使用 compute_gradient(field, coords, component, spatial_dim=2)
    """
    return compute_gradient(field, coords, component, spatial_dim=2)
...
def compute_gradient_3d(
    ...
    ⚠️ Deprecated: 建議使用 compute_gradient(field, coords, component, spatial_dim=3)
    """
    return compute_gradient(field, coords, component, spatial_dim=3)
