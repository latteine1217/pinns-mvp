#!/bin/bash
# ===================================================================
# A2 Ablation Weights 實驗 - Colab 執行腳本
# ===================================================================
# 實驗目的：測試 Adaptive Weighting 對模型效能的影響
# 配置數量：2 個 (With Adaptive vs Without Adaptive)
...
echo -e "${GREEN}================================${NC}"
echo -e "${GREEN}🚀 A2 Ablation Weights 實驗${NC}"
echo -e "${GREEN}================================${NC}"
echo ""
...
# 實驗配置
EXPERIMENT_DIR="configs/experiments/A2_ablation_weights"
CONFIGS=("with_adaptive" "without_adaptive")
K_VALUE=100

# 日誌設置
LOG_DIR="logs/experiments/A2_ablation_weights"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/a2_run_$(date +%Y%m%d_%H%M%S).log"

echo -e "${BLUE}實驗配置：${NC}"
echo "  - Ablation 目標：Adaptive Weighting"
echo "  - 配置數量：${#CONFIGS[@]}"
...
for config_name in "${CONFIGS[@]}"; do
    CONFIG_FILE="$EXPERIMENT_DIR/a2_${config_name}_K${K_VALUE}_2d_re50.yml"
    ...
    if [ -n "$COLAB_EPOCHS" ]; then
        TEMP_CONFIG="/tmp/a2_${config_name}_temp.yml"
    ...
    if python scripts/train/train.py --cfg "$ACTUAL_CONFIG" 2>&1 | tee -a "$LOG_FILE"; then
        ...
        if [ -d "/content/drive/MyDrive" ]; then
            BACKUP_DIR="/content/drive/MyDrive/pinns_checkpoints/A2_${config_name}"
            mkdir -p "$BACKUP_DIR"
            cp -r "checkpoints/experiments/A2_${config_name}_K${K_VALUE}" "$BACKUP_DIR/" 2>/dev/null && \
...
echo "📁 Checkpoints:"
for config_name in "${CONFIGS[@]}"; do
    CKPT_DIR="checkpoints/experiments/A2_${config_name}_K${K_VALUE}"
    if [ -d "$CKPT_DIR" ]; then
        echo "  ✓ $CKPT_DIR"
    else
        echo "  ✗ $CKPT_DIR (未找到)"
...
echo "  --checkpoints \\"
echo "    checkpoints/experiments/A2_with_adaptive_K${K_VALUE}/best_model.pth \\"
echo "    checkpoints/experiments/A2_without_adaptive_K${K_VALUE}/best_model.pth \\"
echo "  --labels \"With Adaptive\" \"Without Adaptive\" \\"
echo "  --output results/A2_ablation_weights_comparison.png"
