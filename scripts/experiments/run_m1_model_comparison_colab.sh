#!/bin/bash
# ===================================================================
# M1 Model Comparison 實驗 - Colab 執行腳本
# ===================================================================
# 實驗目的：比較 Full Model vs Vanilla Model 效能差異
# 配置數量：2 個 (Full vs Vanilla)
...
echo -e "${GREEN}================================${NC}"
echo -e "${GREEN}🚀 M1 Model Comparison 實驗${NC}"
echo -e "${GREEN}================================${NC}"
echo ""
...
# 實驗配置
EXPERIMENT_DIR="configs/experiments/M1_model_comparison"
CONFIGS=("full" "vanilla")
K_VALUE=100

# 日誌設置
LOG_DIR="logs/experiments/M1_model_comparison"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/m1_run_$(date +%Y%m%d_%H%M%S).log"

echo -e "${BLUE}實驗配置：${NC}"
echo "  - 比較目標：Full Model vs Vanilla Model"
echo "  - 配置數量：${#CONFIGS[@]}"
...
for config_name in "${CONFIGS[@]}"; do
    CONFIG_FILE="$EXPERIMENT_DIR/m1_${config_name}_K${K_VALUE}_2d_re50.yml"
    
    echo -e "${YELLOW}================================${NC}"
    echo -e "${YELLOW}▶ 開始訓練: ${config_name^^} Model${NC}"
    echo -e "${YELLOW}================================${NC}"
    ...
    if [ -n "$COLAB_EPOCHS" ]; then
        TEMP_CONFIG="/tmp/m1_${config_name}_temp.yml"
    ...
    if python scripts/train/train.py --cfg "$ACTUAL_CONFIG" 2>&1 | tee -a "$LOG_FILE"; then
        ...
        echo ""
        echo -e "${GREEN}✅ ${config_name^^} Model 訓練完成${NC}"
        echo "耗時: $((EXP_DURATION / 3600))h $((EXP_DURATION % 3600 / 60))m"
        SUCCESS_LIST+=("$config_name")
        
        # 備份到 Google Drive
        if [ -d "/content/drive/MyDrive" ]; then
            BACKUP_DIR="/content/drive/MyDrive/pinns_checkpoints/M1_${config_name}"
            mkdir -p "$BACKUP_DIR"
            cp -r "checkpoints/experiments/M1_${config_name}_K${K_VALUE}" "$BACKUP_DIR/" 2>/dev/null && \
        ...
        echo ""
        echo -e "${RED}❌ ${config_name^^} Model 訓練失敗${NC}"
        echo "耗時: $((EXP_DURATION / 3600))h $((EXP_DURATION % 3600 / 60))m"
        FAILED_LIST+=("$config_name")
    fi
...
echo "📁 Checkpoints:"
for config_name in "${CONFIGS[@]}"; do
    CKPT_DIR="checkpoints/experiments/M1_${config_name}_K${K_VALUE}"
    if [ -d "$CKPT_DIR" ]; then
        echo "  ✓ $CKPT_DIR"
    else
        echo "  ✗ $CKPT_DIR (未找到)"
...
echo "  --checkpoints \\"
echo "    checkpoints/experiments/M1_full_K${K_VALUE}/best_model.pth \\"
echo "    checkpoints/experiments/M1_vanilla_K${K_VALUE}/best_model.pth \\"
echo "  --labels \"Full Model\" \"Vanilla Model\" \\"
echo "  --output results/M1_model_comparison.png"
