#!/bin/bash
# ===================================================================
# S1 Sensor Strategy 實驗 - Colab 執行腳本
# ===================================================================
# 實驗目的：比較 QR-Pivot vs Random 感測器策略
# 配置數量：2 個
# 預估時間：4-8 小時（K=100, 2 個策略）
# ===================================================================
...
echo -e "${GREEN}================================${NC}"
echo -e "${GREEN}🚀 S1 Sensor Strategy 實驗${NC}"
echo -e "${GREEN}================================${NC}"
echo ""
...
# 日誌設置
LOG_DIR="logs/experiments/S1_sensor_strategy"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/s1_run_$(date +%Y%m%d_%H%M%S).log"
...
for strategy in "${STRATEGIES[@]}"; do
    ...
    echo -e "${YELLOW}================================${NC}"
    echo -e "${YELLOW}▶ 開始訓練: ${strategy^^} 策略${NC}"
    echo -e "${YELLOW}================================${NC}"
    ...
    if [ -n "$COLAB_EPOCHS" ]; then
        TEMP_CONFIG="/tmp/s1_${strategy}_temp.yml"
    ...
    if python scripts/train/train.py --cfg "$ACTUAL_CONFIG" 2>&1 | tee -a "$LOG_FILE"; then
        ...
        if [ -d "/content/drive/MyDrive" ]; then
            BACKUP_DIR="/content/drive/MyDrive/pinns_checkpoints/S1_${strategy}"
            mkdir -p "$BACKUP_DIR"
...
echo "  --labels \"QR-Pivot\" \"Random\" \\"
echo "  --output results/S1_sensor_strategy_comparison.png"
