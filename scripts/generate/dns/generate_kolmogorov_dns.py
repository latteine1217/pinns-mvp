...
class KolmogorovFlowDNS:
    ...
    def __init__(
        ...
        # 使用縮小版本: U(y) = alpha * (A / (nu * k_f^2)) * sin(k_f * y)
        # 策略: 使用較弱初始場 (alpha < 1) + 後續擾動，避免數值不穩定但能激發湍流
        alpha_formula = nu * k_f**2 / A
        alpha = max(0.4, min(1.0, alpha_formula))  # 保守值 0.4-1.0
        U_amp = alpha * A / (nu * k_f**2)  # ✅ 修正變數名：U_amp → U_amp
        
        if self.use_torch:
            U_init = U_amp * torch.sin(k_phys * self.Y)
            self.U_hat = self.fft2(U_init)
            self.V_hat = torch.zeros((N, N), dtype=torch.complex64, device=self.device)
        else:
            U_init = U_amp * np.sin(k_phys * Y)
    ...
    def run(
        ...
        if perturbation_times is None:
            ...
            logging.info("ℹ️  未指定擾動時間，預設在 t=5.0s 注入一次擾動")

        # 自動設置擾動振幅（針對 Re=100 使用溫和擾動）
        if perturbation_amplitude is None:
            U0 = self.to_numpy(self.ifft2(self.U_hat)).max()
            V0 = self.to_numpy(self.ifft2(self.V_hat)).max()
            base_velocity = max(U0, V0, 0.1)  # 取 U/V 中較大者，最小 0.1
            ...
            logging.info(f"📊 初始速度 U0={U0:.4f}, V0={V0:.4f}, 自動設置擾動振幅={perturbation_amplitude:.4f} (0.5×max(U0,V0), 上限0.8)")
