...
def compute_gradients(model, x, y, z):
    """計算一階導數 (用於剪應力和質量守恆)"""
...
def validate_wall_boundary(model, n_points=100):
    """驗證壁面邊界條件"""
    print("\n" + "="*70)
    print("🧱 壁面邊界條件驗證")
    print("="*70)
...
def validate_wall_shear_stress(model, physics_config, n_points=100):
    """驗證壁面剪應力"""
    print("\n" + "="*70)
    print("📐 壁面剪應力驗證")
    print("="*70)
    ...
    for wall_name, y_val in [("下壁面 (y=-1)", -1.0), ("上壁面 (y=+1)", 1.0)]:
        ...
        grads = compute_gradients(model, x_wall, y_wall, z_wall)
...
def validate_mass_conservation(model, n_points=50):
    """驗證質量守恆"""
    print("\n" + "="*70)
    print("🌊 質量守恆驗證 (連續性方程)")
    print("="*70)
    ...
    # 在內部區域生成測試點
    x = torch.linspace(0, 8*np.pi, n_points)
    y = torch.linspace(-0.9, 0.9, n_points)  # 避開邊界
    z = torch.linspace(0, 3*np.pi, n_points)
    
    x_test = torch.rand(n_points**2) * 8 * np.pi
    y_test = torch.rand(n_points**2) * 1.8 - 0.9
    z_test = torch.rand(n_points**2) * 3 * np.pi
    
    grads = compute_gradients(model, x_test, y_test, z_test)
...
def validate_velocity_statistics(model, n_points=100):
    """驗證速度場統計特性"""
    print("\n" + "="*70)
    print("📊 速度場統計驗證")
    print("="*70)
