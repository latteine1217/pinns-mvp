...
class TestAxisSelectiveFourierFeatures:
    ...
    def test_trainable_parameters(self):
        """測試可訓練模式"""
        config = {'x': [1, 2]}
        fourier_trainable = AxisSelectiveFourierFeatures(
            axes_config=config,
            full_axes_config=config,
            trainable=True
        )
        fourier_frozen = AxisSelectiveFourierFeatures(
            axes_config=config,
            full_axes_config=config,
            trainable=False
        )
    ...
    def test_set_active_frequencies(self):
        ...
        assert fourier.out_dim == original_out_dim  # 維度保持固定
        assert fourier.axes_config == new_config

        # 測試前向傳播仍正常
        x = torch.randn(5, 3)
        output = fourier(x)
        assert output.shape == (5, original_out_dim)
...
class TestFourierFeatureFactory:
    ...
    def test_create_with_unsupported_type(self):
        ...
        with pytest.raises(TypeError):
            FourierFeatureFactory.create(42, in_dim=3)
...
class TestIntegrationScenarios:
    ...
    def test_frequency_annealing_simulation(self):
        ...
        assert fourier.out_dim == 8  # 維度保持固定（2 * len(full_config['x']))

        # 階段 2：中頻
        stage2 = {'x': [1, 2, 4], 'y': []}
        fourier.set_active_frequencies(stage2)
        assert fourier.out_dim == 8
...
class TestEdgeCases:
    ...
    def test_invalid_config(self):
        ...
        # 空字典
        with pytest.raises(ValueError):
            AxisSelectiveFourierFeatures({}, full_axes_config={})

        # 負頻率
        with pytest.raises(ValueError):
            AxisSelectiveFourierFeatures({'x': [1, -2]}, full_axes_config={'x': [1, -2]})
